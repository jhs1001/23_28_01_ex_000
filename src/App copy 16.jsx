...
function LightComponent({name, position}) {
  return (
    ...
    <spotLight position={position} angle={0.15} penumbra={1} color="blue" />
    : null
  )
}
...
function ControlPanel({count, setCount, addObject, addSpotLight, numObjects}) {
  return (
    ...
    <div className="container">
      <Canvas>
        ...
        {objects.map((name, index) => (
          ...
          <LightComponent
            key={name}
            name={name}
            position={name.includes('Spot Light') ? [(index*2.4)-12, 2, 0] : [(index*2.4)-12, 0, 0]}
          />
          :
          <MeshComponent
            key={name}
      ...
      <div className="side-panel">
        ...
        <ControlPanel
          ...
          setCount={setCount}
          addObject={addObject}
          addSpotLight={addSpotLight} // addSpotLight prop을 추가
